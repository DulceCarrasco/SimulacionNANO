...
for(ins in 1:3) {
  ...
  for (iter in 1:tmax) {
    ...
    p$fact <- NULL
    clusterExport(cls, "p")
    mut <- foreach(i=1:tam, .combine = rbind) %dopar% utacion2() #mutacion paralela
    p <- rbind(p, mut)
...
gg + geom_line(aes(y=No_correlacionado), color="black", size=1.3) + 
  geom_line(aes(y=Correlacionado), color="green", size=1.3) + 
  geom_line(aes(y=Inverso_correlacionado), color="purple", size=1.3) + 
  scale_color_discrete(name = "Fuerzas", labels = c("black", "green", "purple")) +
  ylab("Tiempo") + 
  xlab("Pasos") +
  theme_light()
